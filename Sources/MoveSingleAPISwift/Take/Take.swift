...
public actor Take: Identifiable, Equatable {
    ...
    public func createJob(metadata: Metadata? = nil, mocapModel: String) async throws {
        let metadataJSONString = metadata?.toJSONString() ?? "{}"
        let jobResult = try await graphQLClient.createJob(takeId: takeID, metadata: metadataJSONString, mocapModel: mocapModel)
